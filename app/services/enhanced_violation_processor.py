"""
Enhanced Traffic Violation Detection Processor
Based on New21.ipynb with advanced features:
- License plate detection with EasyOCR
- Accident detection with TFLite
...
class EnhancedViolationProcessor:
    def __init__(self):
        self.model = None
        self.svm_model = None
        self.pca = None
        ...
        self.accident_interpreter = None
        self.accident_input_details = None
        self.accident_output_details = None
        self.plate_regex = re.compile(r'^[A-Z0-9]{6,}$')
        
        # Image processing constants
        self.IMG_HEIGHT = 250
        self.IMG_WIDTH = 250
        self.TFLITE_MODEL_PATH = os.path.join(settings.BASE_DIR, 'tf_lite_model.tflite')
        self.SVM_MODEL_PATH = os.path.join(settings.BASE_DIR, 'svm_model.pkl')
        self.PCA_MODEL_PATH = os.path.join(settings.BASE_DIR, 'pca.pkl')
    ...
    def _load_models(self):
        ...
        try:
            ...
            if os.path.exists(self.SVM_MODEL_PATH):
                ...
                print(f"✅ SVM model loaded from {self.SVM_MODEL_PATH}")
            else:
                print(f"⚠️ SVM model not found at {self.SVM_MODEL_PATH}")
            ...
            if os.path.exists(self.PCA_MODEL_PATH):
                ...
                print(f"⚠️ PCA model not found at {self.PCA_MODEL_PATH}")
    ...
    def _initialize_new_components(self):
        ...
        try:
            ...
            # Initialize TFLite accident detector
            if os.path.exists(self.TFLITE_MODEL_PATH):
                self.accident_interpreter = tf.lite.Interpreter(model_path=self.TFLITE_MODEL_PATH)
                self.accident_interpreter.allocate_tensors()
                self.accident_input_details = self.accident_interpreter.get_input_details()
                self.accident_output_details = self.accident_interpreter.get_output_details()
                
                # Dynamically get input size
                _, self.IMG_HEIGHT, self.IMG_WIDTH, _ = self.accident_input_details[0]['shape']
                print(f"✅ TFLite accident detector loaded from {self.TFLITE_MODEL_PATH}. Input size: {self.IMG_HEIGHT}x{self.IMG_WIDTH}")
            else:
                print(f"⚠️ TFLite accident model not found at {self.TFLITE_MODEL_PATH}. Accident detection disabled.")
    ...
    # --- Accident Detection ---
    class TFLiteAccidentDetector:
        def __init__(self, interpreter, input_details, output_details, check_interval=10):
            self.interpreter = interpreter
            self.input_details = input_details
            self.output_details = output_details
            self.check_interval = check_interval
            self.frame_counter = 0
            self.is_accident_detected = False
            self.last_prob = 0.0
        ...
        def predict_frame(self, frame):
            ...
            resized_frame = tf.image.resize(frame, (250, 250))
            ...
            if (prediction_prob < 1e-4) or (0.05 < prediction_prob < 0.6) or (prediction_prob > 0.95):
                predicted_label = "No Accident"
            else:
                predicted_label = "Accident Detected"

            return predicted_label, prediction_prob

        def update_and_check(self, frame):
            self.frame_counter += 1
            if self.interpreter is None or self.frame_counter % self.check_interval != 0:
                return False
            ...
            if label == "Accident Detected":
                if not self.is_accident_detected:
                    self.is_accident_detected = True
                    print(f"💥 Accident Scene Detected! (Confidence: {prob:.4f})")
                    return True
            else:
                self.is_accident_detected = False
            return False
    ...
    async def process_video_enhanced(self, input_path: str, output_path: str, 
        ...
        if self.accident_interpreter:
            accident_detector = self.TFLiteAccidentDetector(
                self.accident_interpreter, 
                self.accident_input_details, 
                self.accident_output_details, 
                check_interval=15
            )
        ...
        while cap.isOpened():
            ...
            if accident_detector:
                is_new_accident = accident_detector.update_and_check(frame)
                if is_new_accident:
                    stats['accident_scene_detections'] += 1
            ...
            self._annotate_frame(frame, vehicles, traffic_lights, light_colors, 
                               ...
                               wrong_way_ids, accident_detector)
        ...
        print(f"Accident Events: {stats['accident_scene_detections']}")
    ...
    def _annotate_frame(self, frame, vehicles, traffic_lights, light_colors, 
                       red_light_violators, helmet_violators, wrong_way_ids, accident_detector):
        ...
        if accident_detector and accident_detector.is_accident_detected:
